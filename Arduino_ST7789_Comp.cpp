...
static const uint8_t PROGMEM 
  init_240x240[] = {  // Initialization commands for 7789 screens
...
#if defined (SPI_HAS_TRANSACTION)
#define SPI_START()    if (_hwSPI)    SPI.beginTransaction(mySPISettings)
#define SPI_END()      if (_hwSPI)    SPI.endTransaction()
#else
#define SPI_START()    (void)
#define SPI_END()      (void)
#endif
...
// draw Image of camera esp32cam
void Arduino_ST7789::drawEsp32camImage(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t *img8) 
{
  if(x>=_width || y>=_height || w<=0 || h<=0) return;
  setAddrWindow(x, y, x+w-1, y+h-1);
...
{
  writeCmd(mode ? ST7789_INVON : ST7789_INVOFF); 
}
